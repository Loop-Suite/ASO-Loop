...
#[command(
    name = "aso",
    version,
    about = "CLI that generates app store listing copy (title/subtitle/keywords/description) using Claude Code (`claude -p`) as the backend and scores it with a rubric"
)]
struct Cli {
    ...
    model: Option<String>,
    /// Judge model. If multiple are given comma-separated, they are used as a rotating panel (e.g. sonnet,haiku)
    #[arg(long, global = true)]
    ...
    /// Load CLAUDE.md, plugins, and hooks from the working directory (blocked by default via --safe-mode)
    #[arg(long, global = true)]
    load_context: bool,
...
enum Cmd {
    /// Generate N listing copy drafts + score + ranking report
    Gen {
        #[arg(long)]
        spec: PathBuf,
        /// Brief file containing app overview, features, target users, etc. (md/txt)
        #[arg(long)]
        brief: PathBuf,
        ...
        out: PathBuf,
        /// Number of scoring passes per document (truncated mean after rotating through models/perspectives)
    ...
    /// Only score existing listing copy
    Score {
        #[arg(long)]
        spec: PathBuf,
    ...
    Loop {
        ...
        target: f64,
        /// Max iterations. Defaults to 4 since most of the gains reportedly occur within the first 1-2 rounds
        ...
        min_delta: f64,
        /// Stops early if stagnation occurs this many times in a row
        #[arg(long, default_value_t = 2)]
        patience: usize,
        /// Approach angle for the initial draft (uses the spec's default if unspecified)
        #[arg(long, default_value = "")]
        angle: String,
        /// Validation judge model that does not participate in the loop. After completion, re-scores the first draft vs. best draft with this model
        #[arg(long)]
...
fn judge_panel(cli: &Cli) -> Vec<Llm> {
    match &cli.judge_model {
        Some(list) => list.split(',').map(|s| s.trim()).filter(|s| !s.is_empty()).map(|m| build_llm(cli, Some(m.to_string()))).collect(),
...
fn real_main() -> Result<()> {
    ...
    if cli.judge_model.is_none() {
        eprintln!(
            "Warning: the generation model and judge model are the same. Since there's a bias toward rating one's own writing style favorably, \
             it's better to specify a different model with --judge-model."
        );
    }
    ...
    match &cli.cmd {
        Cmd::Gen { spec, brief, count, out, rounds, concurrency, no_score } => {
            ...
            println!("Generating {} items — {} ({})", count, sp.name, sp.store.label());
            ...
            let (docs, failed) = par_map(*concurrency, items, |(i, angle)| {
                ...
                println!("  Generated: {} ({} characters)", label, d.chars().count());
                Ok((label, d))
            });
            if failed > 0 {
                eprintln!("Warning: {failed} generation(s) failed (out of {requested} requested, {} succeeded)", docs.len());
            }
        ...
        Cmd::Score { spec, input, out, rounds, concurrency } => {
            ...
            println!("Scoring {} items — {} ({})", files.len(), sp.name, sp.store.label());
            ...
            // The score mode has no --brief to compare against, so the factual consistency check is skipped.
            let scored = score_many(&judges, &sp, docs, *rounds, *concurrency, &out_dir, None);
            finish(&out_dir, &sp, &scored)
        }

        Cmd::Loop { spec, brief, out, target, max_iter, rounds, min_delta, patience, angle, gate_model } => {
            let sp = Spec::load(spec)?;
            let brief_text = read_text(brief)?;
            let out_dir = prepare_out(out)?;
            let angle = if angle.is_empty() { generate::angles_for(&sp, 1).remove(0) } else { angle.clone() };
            let cfg = loop_run::LoopCfg { target: *target, max_iter: *max_iter, rounds: *rounds, min_delta: *min_delta, patience: *patience };
            println!("Loop started — target {:.0} points, max {} iterations", target, max_iter);
            let r = loop_run::run(&gen_llm, &judges, &sp, &brief_text, &out_dir, &cfg, &angle)?;

            // held-out gate: re-score only the first draft and the best draft using a model that did not participate in the loop
            let mut gate_pair: Option<(Scored, Scored)> = None;
            if let Some(gm) = gate_model {
                println!("held-out validation ({gm})…");
            ...
            let path = report::write_loop_report(&out_dir, &sp, &r.history, &r.stop_reason, &r.warnings, gate_pair.as_ref().map(|(f, b)| (f, b)))?;
            println!("\nFinished: {} · best {:.1}/100 ({})", r.stop_reason, r.best_score.total, r.best_label);
            for w in &r.warnings {
                println!("  ⚠ {}", w);
            }
...
fn score_many(judges: &[Llm], sp: &Spec, docs: Vec<(String, String)>, rounds: usize, concurrency: usize, out_dir: &Path, brief: Option<&str>) -> Vec<Scored> {
    ...
    if failed > 0 {
        eprintln!("Warning: {failed} scoring(s) failed (out of {requested} requested, {} succeeded)", scored.len());
    }
    for s in &scored {
...
/// Runs in parallel, batched by `concurrency`. Failed items are skipped after logging a warning, and
/// the failure count is returned to the caller (does not abort entirely — it's the caller's responsibility to decide whether everything failed).
...
fn collect_docs(input: &Path) -> Result<Vec<PathBuf>> {
    ...
    let mut v: Vec<PathBuf> = std::fs::read_dir(input)
        ...
        .filter(|p| p.is_file() && p.extension().map(|e| e == "md" || e == "txt").unwrap_or(false) && p.file_name().map(|n| n != "report.md").unwrap_or(true))
        .collect();
    v.sort();
    Ok(v)
}
