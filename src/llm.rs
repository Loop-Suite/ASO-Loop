...
///
/// Applies the following by default (all verified via `claude --help`):
/// - `--safe-mode`      : disables CLAUDE.md / skills / plugins / hooks / MCP (auth still works normally)
/// - `--tools ""`       : fully disables built-in tools → pure text generation, no file access
/// - `--no-session-persistence` : no session file created (avoids contention during parallel execution)
/// - `--output-format json`     : collects result / structured_output / total_cost_usd
///
/// Do not use `--bare`. It doesn't read OAuth/keychain, which breaks auth for subscription-login users.
#[derive(Clone, Debug)]
pub struct Llm {
    pub bin: String,
...
impl Llm {
    ...
    fn call_once(
        ...
        // Writing to stdin and reading stdout/stderr must happen concurrently to avoid deadlock from pipe buffer saturation.
            ...
            match child.try_wait()? {
                ...
                None => {
                    if started.elapsed() > self.timeout {
                        let _ = child.kill();
                        let _ = child.wait();
                        return Err(anyhow!("timed out after {} seconds", self.timeout.as_secs()));
                    }
